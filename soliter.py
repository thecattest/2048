...
while running:
	...
	for event in pygame.event.get():
		if event.type == pygame.QUIT:
			running = False
		if event.type == pygame.MOUSEBUTTONDOWN and lose is False:
			k = pygame.mouse.get_pos()
			if k[0] >= 120 and k[0] <= 180 and k[1] >= 390 and k[1] <= 470:
				f = 1
		if event.type == pygame.MOUSEBUTTONUP and lose is False:
			f = 0
			print(all_cards)
			...
			if (len(first_col) + len(second_col) + len(third_col) + len(fourth_col)) == 32 and count_discard == 2:
				if b != first_col[-1] and b != second_col[-1] and b != third_col[-1] and b != fourth_col[-1]:
					...
					text = g.render("YOU LOSE(((", True, (50, 50, 100))
					screen.blit(text, (160, 507))
